...
class BertModel(nn.Module):
    """
    sequence_input: [batchsize, src_len, vocab_dim]
...
class BertForMaskedLM(nn.Module):
    ...
    def forward(self,
        ...
        all_encoder_outputs = self.bert(
            input_ids=input_ids)
        sequence_output = all_encoder_outputs  # 取Bert最后一层的输出

        prediction_scores = self.classifier(sequence_output).permute(1,0,2)

        return prediction_scores  # [src_len, batch_size, vocab_size]


def loss_func(ori,pred):
    mask_idx = (ori != pred).float()
    loss_type = nn.L1Loss(reduction='none')
    loss = loss_type(ori,pred) * mask_idx 
    loss = loss.sum() / mask_idx.sum()
    return loss
...
if __name__ =='__main__':
    ...
    emb = BertModel(config)
    emb = emb.cuda()
    aaa = torch.rand(2,4000,1).cuda()
    bbb = emb(aaa)
    print(bbb.shape)
    # print(bbb[0].shape)
    # for item in bbb[1]:
